  ...
  app.post('/login',(request,result)=>{
    ...
    app.get('/user', isAuthorized, (req, res) => {
      ...
      app.post('/user', isAuthorized, (request, result) => {
        ...
        let sql = `
            insert into user(id_user, nama_user, email, alamat)
            values ('`+data.id_user+`', '`+data.nama_user+`', '`+data.email+`', '`+data.alamat+`');
        `
      ...
      app.put('/user/:id', isAuthorized, (request, result) => {
        ...
        let sql = `
            update user
            set id_user = '`+data.id_user+`', nama_user = '`+data.nama_user+`', email = '`+data.email+`', alamat = '`+data.alamat+`'
    ...
    app.get('/inventaris', isAuthorized, (req, res) => {
     ...
     app.post('/inventaris/:id/take', (req, res) => {
         ...
         update inventaris
         set stok = stok - 1
         where id_inventaris = '`+req.params.id+`'
        `, (err, result) => {
         if (err) throw err 
